...
template<>
struct TorchArgDef<Rasterization3DGSResult>
{
    static auto to(const Rasterization3DGSResult &r)
    {
        return to_torch_args(
            r.render_colors,
            r.render_alphas,
            r.render_extra_signals,
            r.render_normals,
            r.means2d_absgrad,
            r.batch_ids,
            r.camera_ids,
...
namespace
{
    ...
    )
    {
        ...
        if(distributed)
        {
            ...
            TORCH_CHECK(
                camera_model == CameraModelType::PINHOLE, "distributed=True only supports camera_model='pinhole'"
            );
            ...
            TORCH_CHECK(!lidar_coeffs.has_value(), "distributed=True does not support lidar coefficients");
        ...
        {
            TORCH_CHECK(
                tile_size == 8 || tile_size == 16, "Eval3D rasterization requires tile_size in {8, 16}, got ", tile_size
            );
        }
        else
        {
        ...
        TORCH_CHECK(opacities.dim() == batch_ndim + 1, "opacities must have shape [..., N], got ", opacities.sizes());
        TORCH_CHECK(
        ...
        if(covars.has_value())
        {
            TORCH_CHECK(!with_eval3d && !with_ut, "UT and Eval3D rasterization require quats and scales, not covars");
            ...
            TORCH_CHECK(
                covars.value().sizes() == covars_shape || covars.value().sizes() == covars_matrix_shape,
        ...
        else
        {
            ...
            TORCH_CHECK(
                quats.value().sizes() == quats_shape, "quats must have shape [..., N, 4], got ", quats.value().sizes()
            );
        ...
        {
            TORCH_CHECK(!viewmats_rs.has_value(), "viewmats_rs should be None for global rolling shutter");
        }
        else
        {
            TORCH_CHECK(with_ut, "Rolling shutter requires with_ut=True");
            TORCH_CHECK(viewmats_rs.has_value(), "Rolling shutter requires viewmats_rs");
        }
        if(viewmats_rs.has_value())
        {
            TORCH_CHECK(
        ...
        if(rays.has_value())
        {
            ...
            expected_rays_shape.insert(expected_rays_shape.end(), {C, image_height, image_width, 6});
            const int64_t rays_offset = static_cast<int64_t>(expected_rays_shape.size()) - rays_tensor.dim();
            ...
            TORCH_CHECK(rays_tensor.scalar_type() == at::kFloat, "rays must be torch.float32");
        }
        ...
        if(radial_coeffs.has_value())
        {
            ...
            TORCH_CHECK(
                radial.dim() == batch_ndim + 2
                    && radial.size(batch_ndim) == C
                    && (radial.size(batch_ndim + 1) == 6 || radial.size(batch_ndim + 1) == 4),
        ...
        TORCH_CHECK(!external_distortion_params.has_value() || with_ut, "External distortion requires with_ut=True");
        ...
        if(has_color)
        {
            ...
            {
                const bool per_gaussian = colors_tensor.dim() == batch_ndim + 2 && colors_tensor.size(batch_ndim) == N;
                const bool per_view     = colors_tensor.dim() == batch_ndim + 3
        ...
        TORCH_CHECK(has_color || sh_degree < 0, "sh_degree must be None when colors is None");
        ...
        if(extra_signals.has_value())
        {
            ...
            if(extra_signals_sh_degree >= 0)
            {
                ...
                TORCH_CHECK(
                    (extra_signals_sh_degree + 1) * (extra_signals_sh_degree + 1) <= extra_tensor.size(-2),
                    "extra_signals_sh_degree requires more SH coefficients than provided"
                );
            }
            else
            {
                const bool per_gaussian = extra_tensor.dim() == batch_ndim + 2 && extra_tensor.size(batch_ndim) == N;
                const bool per_view     = extra_tensor.dim() == batch_ndim + 3
        ...
        {
            TORCH_CHECK(
                backgrounds.value().dim() == batch_ndim + 2 && backgrounds.value().size(batch_ndim) == C,
    ...
    at::optional<at::Tensor> normalize_covars_for_3dgs(const at::optional<at::Tensor> &covars)
    {
        if(!covars.has_value())
        {
            return c10::nullopt;
    ...
    )
    {
        ...
        expected_shape.insert(expected_shape.end(), {viewmats.size(batch_ndim), image_height, image_width, 6});
    ...
    at::optional<at::Tensor> append_background_depth_channel(
        const at::optional<at::Tensor> &backgrounds, const at::Tensor &means, int64_t C, bool had_features
    )
    {
        if(!backgrounds.has_value())
        {
        ...
        if(had_features)
        {
            at::Tensor zeros = at::zeros_like(backgrounds_tensor.narrow(-1, 0, 1));
    ...
    std::tuple<at::Tensor, at::Tensor> postprocess_render_colors(
        ...
        bool has_extra_signals,
        bool append_depth,
        bool expected_depth,
        int64_t primary_channels,
        int64_t extra_signal_channels
    )
    {
        ...
        TORCH_CHECK(
            render_colors.size(-1) >= primary_channels + extra_signal_channels + (append_depth ? 1 : 0),
        ...
        {
            render_extra_signals = render_colors.slice(-1, primary_channels, primary_channels + extra_signal_channels);

            if(append_depth)
            {
                at::Tensor render_depth = render_colors.slice(-1, depth_start, depth_start + 1);
                if(expected_depth)
                {
                    render_depth = render_depth / render_alphas.clamp_min(1e-10);
                }
                final_render_colors = at::cat({render_colors.slice(-1, 0, primary_channels), render_depth}, -1);
            }
            else
            {
        ...
        else if(expected_depth)
        {
            at::Tensor render_depth
                = render_colors.slice(-1, depth_start, depth_start + 1) / render_alphas.clamp_min(1e-10);
            final_render_colors = at::cat({render_colors.slice(-1, 0, depth_start), render_depth}, -1);
        }
...
)
{
    ...
    if(distributed)
    {
        DistributedCameraGather gathered
            = gather_cameras_for_distributed(viewmats, Ks, local_N, local_C, world_size, process_group_name.value());
...
#    endif
    ...
    if(packed)
    {
        projected_opacities = opacities.view({B, N}).index({batch_ids, gaussian_ids});
    ...
    const bool fused_eligible = !packed
                             && !distributed
                             && has_color
                             && sh_degree >= 0
                             && colors.has_value()
    ...
    if(fused_eligible)
    {
        ...
        if(extra_signals.has_value())
        {
            ...
            if(es.dim() == batch_ndim + 2 && es.size(batch_ndim) == N && es.size(-1) == E)
            {
    ...
    if(append_depth)
    {
        if(fused_assembled)
        {
            render_backgrounds = append_background_depth_channel(backgrounds, means, C, /*had_features=*/true);
        }
        else
        {
            const bool had_features = projected_features.defined();
            projected_features      = append_depth_channel(projected_features, depths, use_hit_distance);
            render_backgrounds      = append_background_depth_channel(backgrounds, means, C, had_features);
        }
    }
    TORCH_CHECK(
        projected_features.defined(), "rasterization_3dgs requires at least one color, extra signal, or depth channel"
    );
    ...
    {
        tile_width  = static_cast<int64_t>(std::ceil(image_width / static_cast<double>(tile_size)));
        tile_height = static_cast<int64_t>(std::ceil(image_height / static_cast<double>(tile_size)));
    }
    ...
    at::optional<at::Tensor> kernel_image_ids    = contiguous_optional(image_ids);
    ...
    for(int64_t start = 0; start < channels; start += channel_chunk)
    {
        ...
        if(with_eval3d)
        {
            RasterizeToPixelsFromWorld3DGSResult raster = call_torch_op<&rasterize_to_pixels_from_world_3dgs>(
...
template<>
struct TorchArgDef<Rasterization2DGSResult>
{
    static auto to(const Rasterization2DGSResult &r)
    {
        return to_torch_args(
            r.render_colors,
            r.render_alphas,
            r.render_normals,
            r.render_normals_from_depth,
            r.render_distort,
            r.render_median,
            r.means2d_absgrad,
            r.camera_ids,
...
namespace
{
    ...
    {
        return mode == "RGB" || mode == "RGB-d" || mode == "RGB-Ed" || mode == "RGB+D" || mode == "RGB+ED";
    }
    ...
    std::vector<int64_t> batch_shape_with_2dgs(const at::Tensor &means, std::initializer_list<int64_t> suffix)
    {
        std::vector<int64_t> shape;
        const int64_t batch_ndim = means.dim() - 2;
        shape.reserve(batch_ndim + static_cast<int64_t>(suffix.size()));
    ...
    )
    {
        ...
        TORCH_CHECK(
            quats.size(-2) == N && quats.size(-1) == 4, "quats must have shape [..., N, 4], got ", quats.sizes()
        );
        TORCH_CHECK(
        ...
        TORCH_CHECK(Ks.size(-2) == 3 && Ks.size(-1) == 3, "Ks must have shape [..., C, 3, 3], got ", Ks.sizes());
        if(sh_degree >= 0)
        {
            TORCH_CHECK(
                colors.dim() == 3 && colors.size(0) == N,
                "SH coefficients must have shape [N, K, D], got ",
                colors.sizes()
            );
            TORCH_CHECK(
...
)
{
    ...
    const bool compute_normals_from_depth = render_mode_has_depth(render_mode) && has_color;
    ...
    check_rasterization_2dgs_inputs(
        means, quats, scales, opacities, colors, viewmats, Ks, sh_degree_value, distloss, append_depth
    );
    ...
    if(packed)
    {
        ...
        projected_opacities  = opacities.reshape({B, N}).index({batch_ids, gaussian_ids});
        image_ids            = batch_ids * C + camera_ids;
        batch_ids_opt        = batch_ids;
        camera_ids_opt       = camera_ids;
        gaussian_ids_opt     = gaussian_ids;
    ...
    at::Tensor isect_offsets
        = call_torch_op<&intersect_offset>("gsplat::intersect_offset", isects.isect_ids, I, tile_width, tile_height);
    isect_offsets = isect_offsets.reshape(batch_shape_with_2dgs(means, {C, tile_height, tile_width}));
    ...
    if(append_depth)
    {
        if(has_color)
        {
            ...
            {
                raster_backgrounds
                    = at::cat({backgrounds.value(), at::zeros_like(backgrounds.value().narrow(-1, 0, 1))}, -1);
            }
        }
        else
        {
    ...
    if(expected_depth)
    {
        ...
        at::Tensor expected    = render_colors.narrow(-1, channels - 1, 1) / render_alphas.clamp_min(1e-10);
        render_colors = channels > 1 ? at::cat({render_colors.narrow(-1, 0, channels - 1), expected}, -1) : expected;
    }
    ...
    at::Tensor camtoworlds_rotation = at::linalg_inv(viewmats).narrow(-2, 0, 3).narrow(-1, 0, 3); // [..., C, 3, 3]
